...
#Swarm
	...
	STAT_UPDATE_TIME=120
	args=$NUM_FOOD
	args="$args $NUM_BOTS"
	#Load gazebo with arena world
		gnome-terminal -e "bash -c \"roslaunch swarm_simulator soup.launch gui:=true ; exec bash\""
	...
	#Load the statistics program
		agent_id=0
		stats_command="th async/SwarmbotGazebo-DQN/statistics.lua "
		stats_command="$stats_command $STAT_UPDATE_TIME $agent_id"
	...
	#Run the Atari code
  	th main.lua -threads $NUM_BOTS -zoom 4 -env async/SwarmbotGazebo-DQN/GazeboEnv -modelBody async/SwarmbotGazebo-DQN/SwarmbotModel -histLen 4 -async A3C -entropyBeta 0 -eta 0.0001 -bootstraps 0 -rewardClip 0 -hiddenSize 512 -doubleQ false -duel false -optimiser sharedRmsProp -steps 4000000 -valFreq 1000 -valSteps 8000 -PALpha 0 -network async/SwarmbotGazebo-DQN/last.weights.t7"$@"
